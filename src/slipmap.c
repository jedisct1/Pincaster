...
int main(void)
{
    ...
    slip_map = new_slip_map((size_t) 6U);
    replace_entry_in_slip_map(&slip_map, "key", 4U, "valueable", 10U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("---\n");
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "value2", 7U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("---\n");
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "va", 2U);
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "val", 4U);    
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "v", 1U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("---\n");
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "valeureux", 10U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("--- (delete)\n");
    remove_entry_from_slip_map(&slip_map, "key2", 5U);
    puts("\n--- (/delete)");
    dump_slip_map(&slip_map);
    puts("---\n");
    replace_entry_in_slip_map(&slip_map, "key2", 5U, "valeureUXZ", 10U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("---\n");
    replace_entry_in_slip_map(&slip_map, "key3", 5U, "value3", 7U);
    puts("\n---");
    dump_slip_map(&slip_map);
    puts("---\n");
    free_slip_map(&slip_map);
    free_slip_map(&slip_map);    
