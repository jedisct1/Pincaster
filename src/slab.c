...
#if 0
static int cb_slot_data(void *context, void *entry, const size_t sizeof_entry)
{
    (void) context;
    (void) sizeof_entry;
    printf("%d\n", * (int *) entry);
...
int main(void)
{
    ...
    int entry = 1;
    unsigned int t = 640U;
    unsigned char *slot_data = NULL;
    
    srand(0U);
    init_slab(&slab, sizeof entry, NULL);
    do {
        if (rand() % 2 == 0 || slot_data == NULL) {
            slot_data = add_entry_to_slab(&slab, &entry);
        } else {
            slot_data = add_entry_to_slab(&slab, &entry);
            remove_entry_from_slab(&slab, slot_data);
            slot_data = add_entry_to_slab(&slab, &entry);            
        }
        entry++;
    } while (--t > 0U);
    slab_foreach(&slab, cb_slot_data, NULL);
    free_slab(&slab, NULL);
